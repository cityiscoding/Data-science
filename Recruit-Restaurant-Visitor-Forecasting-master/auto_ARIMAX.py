...
hol = LD.date_info['holiday_flg'].values
train_hol = hol[:-39].reshape(-1, 1)
test_hol = hol[-39:].reshape(-1, 1)
# print(test_hol)
...
if train:
    ...
    for i in range(N):
        ...
        # fit model
        model = auto_arima(train_data, exogenous=train_hol,
                           start_p=1, start_q=1,
        ...
        answer[i,:] = model.predict(n_periods=39, exogenous=test_hol)
        del model
        gc.collect()
        ...
        if (i+1)%(N/10.) < 1:
            ...
            print('############################################')
            print('############################################')
            print('{:.0%} have done! Congratulation!!!!!'.format((i+1)/N))
            print('############################################')
            print('############################################')
            print('############################################')
            print('############################################')
